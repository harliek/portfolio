...
function buildAtlas(
    ...
    const setFont = (ctx: CanvasRenderingContext2D, px: number) => {
        ...
        } catch { /* Optional feature unavailable. */ }
    }
...
function OriginkitBase_VectorWordmark(props: VectorWordmarkProps) {
    const {
        text = "VECTOR",
        font = {
            fontFamily: "Inter",
...
export default function VectorWordmark(props: Record<string, unknown>) {
  return <OriginkitBase_VectorWordmark {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
